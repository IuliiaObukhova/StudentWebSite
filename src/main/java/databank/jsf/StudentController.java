...
/**
 * <p>
 * Description: Responsible for collection of StudentPojo's in XHTML (list)
 * <h:dataTable> </br>
...
public class StudentController implements Serializable {
	...
	/**
	 * Toggles the add student mode which determines whether the addStudent form is
	 * rendered
	 */
	...
	public String editStudent(StudentPojo student) {
		LOG.error("editStudent = {}", student);
		student.setEditable(true);
		return null; // Stay on current page
	}

	public String updateStudent(StudentPojo studentWithEdits) {
		LOG.error("updateStudent = {}", studentWithEdits);
		StudentPojo studentToBeUpdated = studentDao.readStudentById(studentWithEdits.getId());
		if (studentToBeUpdated == null) {
			LOG.error("FAILED update student, does not exists = {}", studentToBeUpdated);
			// If the student being updated does not exists, send an error message to
			// index.xhtml
			// <h:messages globalOnly="true" layout="table" styleClass="alert
			// alert-danger"/>
			...
			if (studentToBeUpdated == null) {
				LOG.error("FAILED update student = {}", studentToBeUpdated);
